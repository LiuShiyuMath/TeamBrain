...
async function openRealEventLog(eventsDbPath) {
  try {
    ...
    const { SqliteEventLog, openDb } = await import("./src-ZPP6L4CV.js");
...
async function checkCmd() {
  const { fetchRemoteSha } = await import("./github-api-3MFVCIRV.js");
  const s = readState2();
...
` };
  ...
  const result = await fetchRemoteSha({
    ...
    ifNoneMatch: s.last_branch_etag || void 0,
    cachedSha: s.last_branch_sha || void 0
  });
  if (!result.ok) {
    if (result.reason === "rate_limit_anonymous" || result.reason === "rate_limit_authed") {
      ...
      writeState({
        ...
        next_check_after_ts: Date.now() + delayHours * 3600 * 1e3
      });
    }
