...
## TeamAgent 经验（114条活跃知识，为你编译了 26 条（token 预算 3000）)
...
- 验证进度时优先用 `git log --oneline` / `git status` 直接查询，信任 git 历史而非口头报告——git 是客观事实记录，commit 历史不会谎报；teammate 状态报告可能滞后、遗漏或未及时同步。在分布式协作中，git 是最可靠的完成度和进度指标 [0.95]
...
- 在停止前附加 <laziness-self-report> block，包含 premature_stopping / permission_seeking / ownership_dodging / simplest_fix / reasoning_loop / known_limitation 六个字段的诚实自我评估——Stop hook 要求在每条消息末尾附加 laziness-self-report block；缺少该 block 属于格式违规，即使内容正确也构成问题，因为格式完整性是协作规范的一部分 [0.90]
- 使用 消息末尾必须附加完整 <laziness-self-report> block，包含 6 个布尔字段：premature_stopping, permission_seeking, ownership_dodging, simplest_fix, reasoning_loop, known_limitation 而非 <laziness-self-report>——用户明确要求每条消息末尾带 self-report block 供 hook 校验，遗漏或格式不完整都会被 catch 并打回补全 [0.90]
- 消息末尾附加完整的 <laziness-self-report> block，包含全部 6 个布尔字段的诚实自评——Stop hook 强制每条消息末尾都有格式化的自评 block；缺少该 block 属于格式违规会被 hook 拒收；这是协作协议一部分，确保 AI 能进行懒惰模式的诚实评估 [0.90]
- 查源代码路径和 commit 版本确认实现状态，而非凭记忆或假设说'未实现'——分布式实现的功能跨多个包/文件，凭印象说'未实现'容易过时且误导团队；应以代码真相和版本号为准 [0.90]
- 提前盘点行数预算，使用指针模式（父文档仅保留表格指针行，详细内容沉到 docs/features/<name>.md 子文档）——并行 feature 各自增加内容容易超行数预算；指针+沉层保持核心文档精炼可控，完整内容在子文档可寻可读，避免团队间协调冲突 [0.90]
- 在消息末尾严格附加完整的 <laziness-self-report> block，包含 premature_stopping / permission_seeking / ownership_dodging / simplest_fix / reasoning_loop / known_limitation 全部 6 个布尔字段——Stop hook 强制要求每条消息带 self-report block 供校验协议使用，缺少或不完整都会被自动拒收并打回补全，格式完整性是协作规范的一部分 [0.90]
- 在消息末尾附加完整的 <laziness-self-report> block（包含 premature_stopping / permission_seeking / ownership_dodging / simplest_fix / reasoning_loop / known_limitation 六个布尔字段）——Stop hook 强制要求每条消息带格式化的自评 block；缺少该 block 属于格式违规会被 hook 拒收；这是用户协作协议的一部分，确保 AI 能进行懒惰模式的诚实评估 [0.90]
- 逐项用 git log 检查每个预期工作的 commit，不能基于一个未提交区块就推断整体未完成——分布式协作中，不同功能分别 commit，working tree 的单个 pending change（如 CLAUDE.md 自动区块）不代表其他工作也未完成；必须逐项验证 git 历史而非凭单一信号推断全局状态 [0.90]
- 运行 git log 逐行检查被判断的 commit，不要凭工作区快照或记忆下结论——git 历史是唯一可信的完成度源头；工作区快照和记忆会导致漏判，影响下一步决策 [0.90]
- 用 git log --all --graph --decorate 或 git worktree list 查全局状态，包括 detached HEAD 和未推送的 worktree——worktree 和 detached HEAD 中的 commits 是真实工作，仅查 main 分支会看不到，导致误判 teammate 状态。全局查询才能准确确认工作存在位置，避免不必要的应急干预 [0.90]
- 使用 先用 git log 验证目标分支最新状态，确认是否已包含该工作再决策 而非 cherry-pick——AI 未充分验证 v2 分支已包含全部工作（包括 docs/README.md entry），仍等待 cherry-pick 决策。应先用 git log/git show 查证分支完整性，避免建议冗余操作。 [0.90]
- 用 git log 和 teammate 消息验证工作完成，忽略临时 API 错误——API 错误（rate limit、连接超时等）是临时网络状态，不反映实际工作完成；git 历史和 teammate 报告是不可变的事实来源，优先级更高 [0.90]
- 消息末尾附加完整的 <laziness-self-report> block，包含 6 个布尔字段的诚实自评——Stop hook 强制协议：每条消息末尾必须有格式化自评 block 供 hook 验证，缺失或不完整会被拦截，这是实时监管协作规范的一部分 [0.90]
- 在消息末尾附加完整的 <laziness-self-report> block（6 个布尔字段：premature_stopping, permission_seeking, ownership_dodging, simplest_fix, reasoning_loop, known_limitation）——Stop hook 强制格式规范，缺少或不完整的 block 会被自动拒收并打回；这是 TeamBrain 协作协议的基础约束 [0.90]
- 在消息末尾附加格式正确的 <laziness-self-report> block，包含 6 个布尔字段（premature_stopping / permission_seeking / ownership_dodging / simplest_fix / reasoning_loop / known_limitation）——Stop hook 会强制验证每条消息末尾是否有完整、格式正确的 laziness-self-report block；缺失或格式错误会被 hook 拒收并要求重新提交；这是协作规范的实时监管机制 [0.90]
- 消息末尾附加完整 <self-report> block，包含全部 12 个布尔字段：premature_stopping / permission_seeking / ownership_dodging / simplest_fix / reasoning_loop / known_limitation / skipped_repo_search / fabricated_value / placeholder_used / ambiguity_unresolved / contradiction_unresolved / silent_fallback——Stop hook 已升级为验证 12 个字段（原内存记录为 6 个）；缺少任何字段或整个 block 都会被 hook 拒收并打回补全，字段数量变化需更新协作协议认知 [0.90]
- 消息末尾同时附加 <laziness-self-report>（6字段）与 <self-report>（12字段）两个 block，每个字段值必须是字面量 true 或 false，以同时满足两个并存的 Stop hook——项目同时运行两个版本的 Stop hook（6字段版与12字段版），任一缺失都会被拦截；两个 block 同时附加是当前唯一可靠的通过方式 [0.90]
- 消息末尾附加完整 <laziness-self-report> block，6 个字段（premature_stopping / permission_seeking / ownership_dodging / simplest_fix / reasoning_loop / known_limitation）值必须是字面量 true 或 false——Stop hook 同时校验字段存在性与值类型：字段缺失或值不是字面量 true|false 都会被 hook 拒收并打回重提；字段格式合规是消息被接受的必要条件 [0.90]
- 诊断问题后继续修复直到问题彻底解决；不能以识别问题作为工作的结束点——诊断是工作流程的中点，不是终点；真正的完成标准是问题已解决，而非问题已识别。承认问题而不修复等同于把责任转嫁给下一步 [0.90]
...
> 还有 65 条 canonical+ 规则因 token 预算未显示（teamagent compile --dry-run 查看）
> 另有 16 条因与已选条目近义（Jaccard ≥ 0.6）被多样性过滤
<!-- TEAMAGENT:END -->
