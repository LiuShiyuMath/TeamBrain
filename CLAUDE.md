...
## TeamAgent 经验（55条活跃知识，为你编译了 28 条（token 预算 3000）)
...
- 每条 message 末尾都必须附加完整的 12 字段 <self-report> block，每个字段填 true|false——项目 Stop hook（.claude/hooks/self-report-fused.sh）强制检查此规则；缺少或格式错误时 Hook 会拦截并阻止消息发出，是输出合规的必要条件 [0.90]
